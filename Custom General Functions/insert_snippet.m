...
function insert_snippet(snippet_file_name)
  
  if exist('snippet', 'var'), snippet = struct('snippet',snippet); end      % Save the variable SNIPPET, if it exists
  ...
  if ~contains(snippet_file_name, '.m')
    snippet_file_name = [snippet_file_name '.m'];
  end
  snippet.txt = fileread([fileparts(which('insert_snippet')), '\snippets\', snippet_file_name]);
